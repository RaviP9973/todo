...
function TodoLogin() {
  ...
  // function for deep linking for referals 


   const { ref } = useParams(); // ✅ Get :ref from the URL
  ...
  useEffect(() => {
    if (ref) {
      console.log("Ref",ref)
      setReferredBy(ref);
      localStorage.setItem("referred_by", ref); // store temporarily
    }
  }, [ref]);
    ...
    try {
      ...
      if (error) {
        ...
        if (!isLogin) {
          ...
          if (existingUser) {
            console.log("I am an existing user", existingUser);
            ...
            // If referral code is present, update the referred_by field
             
            // console.log("referredby", referredBy);
            // if (referredBy) {
            //   console.log("Signup is successfull, updating referred_by field", referredBy);
      ...
      <div className={`w-full max-w-md rounded-2xl shadow-xl px-3 py-4 md:p-8 space-y-3 md:space-y-6 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'}`}>
        ...
        {!otpSent ? (
          <form className="space-y-4" onSubmit={handleSubmit(onSubmitOtp)}>
            ...
            <button
              type="submit"
              className={`w-full duration-300 font-semibold py-3 rounded-lg transition-all ${!isValid || sendingOtp
